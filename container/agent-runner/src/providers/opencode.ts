...
function buildOpenCodeConfig(options: ProviderOptions): Record<string, unknown> {
  ...
  // Native-creds path (no OneCLI proxy): a real key is forwarded into the
  // container env (host opencode.ts). When present, use it as the provider
  // apiKey instead of the OneCLI 'placeholder'. Currently Google/Gemini via
  // AI Studio; extend this lookup for other native-direct providers.
  const nativeApiKey = process.env.GEMINI_API_KEY || process.env.GOOGLE_GENERATIVE_AI_API_KEY;
  ...
  const providerOptions: Record<string, unknown> =
    provider === 'anthropic'
      ...
      : {
          [provider]: {
            options: {
              apiKey: nativeApiKey || 'placeholder',
